...
public class gameNodeMat {
	...
	ArrayList<gameNodeMat> subNodes = new ArrayList<gameNodeMat>();
	...
	public static void main(String[] args) {
		...
		gameNodeMat tree = new gameNodeMat(rootGame, 0, maxTreeDepth, null);
	 ...
	 */
	public gameNodeMat(game GAME, int DEPTH, int MAXDEPTH, move lastMove) {
		game = GAME;
		depth = DEPTH;
		maxDepth = MAXDEPTH;
		...
		if ( !(game.isGameFinshed()) ) {//if the has possible moves to be played
			if( depth < MAXDEPTH ) {
				for(move move :legalMoves) { // create all subnodes (0 legal moves if game is finished)
					...
					gameNodeMat subNode = new gameNodeMat(subGame, (this.depth+1), maxDepth, move);
 ...
 */
	public int getTotalPossibleGames() {
		if ( subNodes.size() == 0) {
			...
			int sum = 0;
			for(gameNodeMat subNode : subNodes) {
