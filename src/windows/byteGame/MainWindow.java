...
public class MainWindow{
		...
		if( colourOfPlayer == 1 ) {
			for(int row = 0; row<8 ; row++) {
				for(int column = 0; column<8; column++) {
					boardTiles[row][column].setCurrent(func.pieceByteToStr(game.getBoard()[row][column]));
			...
			for(int row = 0; row<8 ; row++) {
				for(int column = 0; column<8; column++) {
					boardTiles[7-row][7-column].setCurrent(func.pieceByteToStr(game.getBoard()[row][column]));
    ...
    private void showPossibleMoves(String startSqStr, ArrayList<byteMove> legalMoves) {
    	...
    	for(byteMove legalMove : legalMoves) {
			...
			for(int file=0; file<8; file++) {
				...
				boardTiles[rank][file].button.addMouseListener(new MouseListener() {					
					...
					public void mouseReleased(MouseEvent e) {
						...
						byteMove userMove = new byteMove(startSquareStr, targetSquareStr);
						if( game.isMoveLegal(userMove)) {
							if( game.isPromotionMove(userMove)) {
				    			...
				    			if( p.getPieceSelected()!=0 ){
				    				game.playMove(userMove, 1, p.getPieceSelected());
				    			}
							}else{
								game.playMove(userMove, 1, (byte)0);
							}
		...
		if( !game.isGameFinshed() ) {
			switch (gamemode) {
				case 1: // v random opponent
					byteMove randomMove = PlayOpponents.getRandomByteMove(game);
					if( randomMove instanceof bytePromotionMove ) {
						bytePromotionMove pMove = (bytePromotionMove)randomMove;
						game.playMove(randomMove, 1, pMove.getPromotionPiece());
					}else {
						game.playMove(randomMove, 1, (byte)0);
					}
					break;
				case 2: // 1 depth 
					byteMove checkmateMove = PlayOpponents.getRandomByteMove(game);
					if( checkmateMove instanceof bytePromotionMove ) {
						bytePromotionMove pMove = (bytePromotionMove)checkmateMove;
						game.playMove(pMove, 1, pMove.getPromotionPiece());
					}else {
						game.playMove(checkmateMove, 1, (byte)0);
					}
					break;
				default:
					break;
